...
在企业信息系统中，数据的准确性至关重要。系统设计必须将控制模型幻觉作为第一优先级，同时充分利用大模型的智能能力：通过本体（Ontology）和知识图谱（Knowledge Graph）增强 LLM 的推理能力，使其能够准确理解业务语义、实体关系和领域知识。

在严格控制幻觉的前提下，LLM 负责生成准确的 Cypher 查询和格式化输出，充分利用其理解和推理能力；绝不直接回答问题，所有答案均基于知识图谱的真实查询结果。
...
优先引入成熟的开源组件和商业解决方案，在其基础上进行集成和定制，而非从零开发。例如引入 HiClaw 作为 Agent 编排框架，NebulaGraph 作为图数据库，站在成熟技术的肩膀上解决问题。

### 3. 用户隔离 — 独立 Session、权限、上下文与 Memory

每个用户拥有独立的 Session、权限、上下文和 Memory，用户之间完全隔离。Approach B 通过为每个用户创建独立的 Matrix 账号实现真正的隔离，确保数据安全和个性化体验。

### 4. 组件解耦，保持升级能力

对开源组件的引用保持松耦合，避免直接修改其内部代码。配置与代码分离，通过环境变量或配置文件管理，保持升级的可能性。

### 5. 融入企业现有系统

系统设计必须与企业现有 IT 系统有机结合，能够融入当前的 ERP、数据仓库、权限系统等企业级基础设施中，才能发挥最大价值。
