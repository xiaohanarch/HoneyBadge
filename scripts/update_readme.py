...
# Pattern to find the old architecture section
pattern = r'### Phase 1 架构图\(Approach B — 当前\)\n\n```\n┌─+.*?```\n\n\*\*Approach B 的核心设计：.*?\*\*\n(?=#|\Z)'

new_section = '''### Phase 1 架构图（Approach B — 当前）
...
if result != content:
    ...
    idx = content.find('### Phase 1 架构图')
    print(f'Section marker found at index: {idx}')
    if idx >= 0:
        print(f'Context around marker: {repr(content[idx:idx+200])}')
